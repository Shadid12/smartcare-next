...
export class ProfileDetails extends React.Component {
  ...
  componentDidMount() {
    ...
    axios.get('https://smartapinode.herokuapp.com/patients/info',
    { headers: {"Authorization" : `Bearer ${token}`} })
    .then((result) => {
      console.log('--->', result.data);
      this.setState({patient: result.data.patient})

        this.state.patient.patientForm.items.map((item) => {
            if(item.q_label === 'First Name') {
                this.setState({
                    firstName: item.answer
                })
            }

            if(item.q_label === 'Last Name') {
                this.setState({
    ...
    .catch(err => {
      console.log('SOmething Wehnt Wrong', err);
    })
  }

  switchView = () => {
    const {dispatch} = this.props;
    dispatch(changePatientProfile(0));
  }
                ...
                <Grid item xs={10}>
                    <Paper className={classes.root} elevation={1}>
                        <Typography variant="h5" component="h3">
                            <IconButton className={classes.iconBtn} onClick={this.switchView}>
                                <FaArrowLeft />
                            </IconButton>
                            Profile Summary
                        </Typography>
            ...
            >
                ...
                <Grid item xs={6}>
                    <Paper className={classes.root} elevation={1}>
                        ...
                        <Button variant="contained" className={classes.changepass}>
                            Change Password
                        </Button>                   
                    </Paper>
                    <Grid item xs={12}>
                        <Paper className={classes.addional} elevation={1}>
...
export default connect(mapStateToProps)(withStyles(styles)(ProfileDetails));